...
class _SamplePageState extends State<SamplePage> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    _getController.controllerConvex = TabController(length: 5, vsync: this); // Update length to match tab items
  }
    ...
    return Obx(() => Scaffold(
      ...
      bottomNavigationBar: ConvexAppBar(
        ...
        items: [
          ...
          TabItem(icon: Container(decoration: const BoxDecoration(color: AppColors.white, shape: BoxShape.circle), child: Container(margin: EdgeInsets.all(5.r), decoration: const BoxDecoration(color: AppColors.blue, shape: BoxShape.circle), child: IconButton(icon: Icon(EneftyIcons.scan_barcode_bold, color: AppColors.white, size: 30.sp), onPressed: () => Get.to(QRViewExample()))))),
