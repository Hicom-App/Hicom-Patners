...
class PartnersView extends GetView<PartnersController> {
  ...
  void _showPartnerDialog(BuildContext context, dynamic partner, int index) {
    showDialog(
        ...
        builder: (BuildContext context) {
          final todayWorkingDay =
              controller.getTodayWorkingDay(partner.workingDays);
          final bool isOpenToday = todayWorkingDay?.isOpen == 1;
          final Color statusColor = todayWorkingDay != null
              ? (isOpenToday ? AppColors.green : AppColors.red)
              : AppColors.red;
          final String statusText = todayWorkingDay != null
              ? (isOpenToday ? 'Ochiq'.tr : 'Yopiq'.tr)
              : 'Yopiq'.tr;
          return AlertDialog(
              ...
              title: Row(children: [
                Container(
                    ...
                    child: ClipRRect(
                        ...
                        child: Image.network(
                            _getController
                                .partnerModels.value.result![index].photoUrl
                                .toString(),
                            fit: BoxFit.cover,
                ...
                Expanded(
                    child: TextSmall(
                        text: partner.name ?? 'Noma’lum do‘kon',
                        fontSize: 16.sp,
                        fontWeight: FontWeight.w600,
                        color: AppColors.black)),
                IconButton(
                    icon: Icon(Icons.close,
                        size: 20.sp, color: AppColors.black70),
                  ...
                  child: Column(
                    ...
                    Row(children: [
                      Icon(Icons.location_on,
                          size: 16.sp, color: AppColors.black70),
                      SizedBox(width: 8.w),
                      SizedBox(
                          width: Get.size.width * 0.6,
                          child: TextSmall(
                              text:
                                  '${'Taxminan'.tr} ${_getController.partnerModels.value.result![index].distance} ${'uzoqlikda'.tr}',
                              fontSize: 14.sp,
              ...
              actions: [
                ElevatedButton(
                    onPressed: () {
                      Get.back();
                      Get.to(() => PartnersDetailView(
                            index,
                            partner.workingDays != null &&
                                    partner.workingDays!.length >
                                        controller.currentWeekday
                                ? partner
                                            .workingDays![
                                                controller.currentWeekday]
                                            .isOpen ==
  ...
  void _showStyledMapMenu(BuildContext context) {
    showMenu<dynamic>(
        // <dynamic> qo'shildi – tur mosligini ta'minlash uchun
        context: context,
        ...
        items: <PopupMenuEntry<dynamic>>[
          // Ro'yxatni explicit tip bilan belgilash
          PopupMenuItem(
              value: false,
              enabled: true,
                ...
                Expanded(
                    child: Column(
                      ...
                      TextSmall(
                          ...
                          color: AppColors.black),
                      TextSmall(
                          text: 'Oddiy ko‘cha xaritasi'.tr,
                          fontSize: 12,
                          color: AppColors.black70)
                    ])),
                if (!controller.isHybridStyle)
                  Icon(Icons.circle, color: AppColors.blue, size: 14.sp)
              ])),
              ...
              child: Row(children: [
                Container(
                    ...
                    child: Icon(Icons.satellite,
                        color: AppColors.blue, size: 24.sp)),
                SizedBox(width: 12.w),
                const Expanded(
                    child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                      ...
                      TextSmall(
                          ...
                          color: AppColors.black),
                      TextSmall(
                          text: 'Sun’iy yo‘ldosh + ko‘chalar',
                          fontSize: 12,
                          color: AppColors.black70)
                    ])),
                if (controller.isHybridStyle)
  ...
  Widget build(BuildContext context) {
    if (!Get.isRegistered<PartnersController>()) {
    ...
    return Obx(() => Scaffold(
        ...
        appBar: CustomAppBar(
            title: 'Hamkorlar'.tr,
            subtitle: 'Yaqin atrofdagi dokonlar'.tr,
            isBack: false,
            isCenter: false,
            icon: controller.viewMode == ViewMode.list
                ? EneftyIcons.map_bold
                : LucideIcons.list,
            suffixOnTap: () {
  ...
  Widget _buildListViewFullPage(
      BuildContext context, Function(String) setSearchQuery) {
    return Stack(children: [
      RefreshComponent(
          ...
          enablePullUp: false,
          onRefresh: () async {
            _getController.clearPartnerModels();
            ApiController().getPartnerMagazine(search: controller.searchQuery.isNotEmpty ? controller.searchQuery : null, rating: controller.sortByRating ? true : null).then((value) {
      ...
      Obx(() => AnimatedPositioned(
          ...
          child: Visibility(visible: controller.showScrollToTop.value, child: FloatingActionButton(shape: const CircleBorder(), mini: true, onPressed: () => controller.scrollController.animateTo(0, duration: const Duration(milliseconds: 500), curve: Curves.easeInOut), backgroundColor: AppColors.blue, child: Icon(Icons.arrow_upward, color: AppColors.white, size: 20.sp)))))
    ]);
  }

  Widget _buildMapView(BuildContext context) => Stack(children: [
        SizedBox(
            width: Get.width,
                ...
                children: [
                  if (controller.isHybridStyle)
                    TileLayer(
                        ...
                        userAgentPackageName: 'partner.hicom.uz'),
                  if (controller.isHybridStyle)
                    TileLayer(
                        urlTemplate:
                  ...
                  Obx(() {
                    ...
                    if (result != null && result.isNotEmpty) {
                      for (var entry in result.asMap().entries) {
                        ...
                        final point = LatLng(
                            partner.lng!.toDouble(), partner.lat!.toDouble());
                        shopMarkers.add(Marker(
                            ...
                            child: GestureDetector(
                                ...
                                child: Container(
                                    ...
                                    child: ClipRRect(
                                        borderRadius: BorderRadius.circular(8),
                                        child: Icon(EneftyIcons.shop_bold,
                                            size: 32.sp,
                                            color: AppColors.blue))))));
                      }
                      print(
                          'Qoshilgan markerlar soni: ${shopMarkers.length}'); // Debug
                    }
                        ...
                        Marker(
                            ...
                            child: Container(
                                decoration: BoxDecoration(
                                    ...
                                    boxShadow: [
                                  BoxShadow(
                                      color: AppColors.grey.withAlpha(400),
                                      spreadRadius: 10,
                                      blurRadius: 1)
                                ]))),
                      ...shopMarkers
        ...
        Obx(() => Container(
            ...
            height: 135.h,
            padding: const EdgeInsets.all(16),
            child: NearestPartnerCard(
                partner: _getController.partnerModels.value))),
        Positioned(
            right: 16.h,
            bottom: 100,
            child: Column(children: [
              ...
              IconButton(
                  ...
                  icon: Icon(LucideIcons.locate,
                      color: AppColors.white, size: 24.0.sp),
        ...
        Positioned(
            ...
            child: Column(children: [
              IconButton(
                  icon: Icon(LucideIcons.layers_2,
                      color: AppColors.white,
                      size: Responsive.scaleFont(24, context)),
                  onPressed: () {
                    _showStyledMapMenu(context);
                  },
                  style: IconButton.styleFrom(
                      backgroundColor: AppColors.blue,
                      shape: RoundedRectangleBorder(
...
class NearestPartnerCard extends GetView<PartnersController> {
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      final todayWorkingDay =
          controller.getTodayWorkingDay(controller.nearest?.workingDays);
      final bool isOpenToday = todayWorkingDay?.isOpen == 1;
      final Color statusColor = todayWorkingDay != null
          ? (isOpenToday ? AppColors.green : AppColors.red)
          : AppColors.red;
      ...
      return InkWell(
          onTap: () => Get.to(() => PartnersDetailView(
                _getController.partnerModels.value.result!.indexOf(nearest),
                nearest.workingDays != null &&
                        nearest.workingDays!.length > controller.currentWeekday
                    ? nearest.workingDays![controller.currentWeekday].isOpen ==
                            1
                        ? 'Ochiq'.tr
                        : 'Yopiq'.tr
                    : '-',
                ...
                Expanded(
                    child: Column(
                      ...
                      Row(children: [
                        ...
                        TextSmall(
                            text: 'Eng yaqin do‘kon'.tr,
                            fontSize: 16,
                            fontWeight: FontWeight.w600,
                            color: AppColors.blue)
                      ]),
                      TextSmall(
                          text: nearest.name ?? 'Noma’lum do‘kon'.tr,
                          fontSize: 16,
                          fontWeight: FontWeight.w500,
                      ...
                      Row(children: [
                        ...
                        SizedBox(width: 4.w),
                        TextSmall(
                            text: '${nearest.distance} ${'uzoqlikda'.tr}',
                            fontSize: 14,
                            color: AppColors.greys2),
                        SizedBox(width: 16.w),
        ...
        child: Row(mainAxisAlignment: MainAxisAlignment.start, children: [
          ...
          Obx(() => GestureDetector(
              ...
              child: Container(
                  ...
                  decoration: BoxDecoration(
                      border: Border.all(
                          color: controller.selectedCategory == 'Barchasi' &&
                                  !controller.showOnlyOpen &&
                                  !controller.sortByRating &&
                                  controller.sortBy == 'distance' &&
                                  controller.selectedCountry.isEmpty &&
                                  controller.selectedRegion.isEmpty &&
          ...
          Obx(() => GestureDetector(
              onTap: () =>
                  controller.toggleShowOnlyOpen(), // Filter/sort chaqirish
              child: Container(
                  padding:
                      const EdgeInsets.symmetric(horizontal: 10, vertical: 6),
                  margin: const EdgeInsets.only(right: 10),
                  decoration: BoxDecoration(
                    border: Border.all(
                        color: controller.showOnlyOpen
                            ? AppColors.blue
                            : Colors.grey),
                    borderRadius: BorderRadius.circular(12),
                    color: controller.showOnlyOpen
                        ? AppColors.blue
                        : Colors.grey[200], // Active holatda ko'k rang
                  ),
                  child: Row(children: [
          ...
          Obx(() => GestureDetector(
              ...
              child: Container(
                  ...
                  decoration: BoxDecoration(
                      border: Border.all(
                          color: controller.isFiltersApplied
                              ? AppColors.blue
                              : Colors.grey),
                      borderRadius: BorderRadius.circular(12),
                      color: controller.isFiltersApplied
            ...
            child: Container(
                ...
                child: Column(
                    ...
                    children: [
                      ...
                      Expanded(
                          child: SingleChildScrollView(
                              child: Column(
                                  ...
                                  children: [
                            _buildFilterSection(
                                'Davlat', _buildCountryFilter()),
                            const SizedBox(height: 24),
                            _buildFilterSection(
                                'Viloyat', _buildRegionFilter()),
                            const SizedBox(height: 24),
                            Obx(() => controller.selectedRegion.isNotEmpty &&
                                    controller.districts.isNotEmpty
                                ? _buildFilterSection(
                                    'Tuman', _buildDistrictFilter())
                      ...
                      Obx(() => Row(
                              mainAxisAlignment:
                                  MainAxisAlignment.center, // Har doim o'rtada
                              children: [
                                Container(
                                    decoration: BoxDecoration(
                                        color: controller.isFiltersApplied
    ...
    return Obx(() => Container(
        ...
        child: DropdownButtonHideUnderline(
            child: DropdownButton<String>(
                menuMaxHeight: 250.h,
                value: controller.selectedCountry.isEmpty
                    ? null
                ...
                elevation: 6,
                hint: Row(children: [
                  Icon(Icons.public, size: 20.sp, color: AppColors.black70),
                  const SizedBox(width: 8),
                  TextSmall(
                ...
                items: controller.countries
                    .map((country) => DropdownMenuItem<String>(
                        value: country == 'Barcha davlatlar' ? '' : country,
                        child: TextSmall(
                            text: country,
                            fontSize: 16.sp,
                            fontWeight: FontWeight.w500,
    ...
    return Obx(() => Column(children: [
          Container(
              ...
              child: DropdownButtonHideUnderline(
                  child: DropdownButton<String>(
                      menuMaxHeight: 250.h,
                      value: controller.selectedRegion.isEmpty
                          ? null
